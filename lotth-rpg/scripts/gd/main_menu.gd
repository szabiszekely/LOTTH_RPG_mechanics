...
@onready var Beta: TextureRect = $LotthBeta
...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_accept") and !_started:
		_started = true
		_Press_start_animation(0.1)
		await get_tree().create_timer(0.5).timeout
		Press_start.queue_free()
		var tween = get_tree().create_tween().set_parallel(true)
		tween.tween_method(set_strength_value, 0.009, 0.15, 1.5); 
		tween.tween_method(set_noise_scale_value, 5.0, 40.0, 1.5); 
		tween.tween_method(set_color_alpha_value, 1.0, 0.0, 3); 
		await tween.finished
		Dialogic.start("a_speaker")
		#
