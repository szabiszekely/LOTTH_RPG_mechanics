	...
	if start_choosing == true:
		...
		if Input.is_action_just_pressed("ui_accept") and start_choosing == true:
			...
			if menu_system.bag == true:
				...
				menu.Initiative._next_in_order() 
				menu.Initiative.action_queued.push_back(["bag", item_againts_players , sub_index ,0])
				_reset_focus()
...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("debug_button_2"):
		...
		player[index].Initiative.action_queued.push_back(["atk",player[index].Fight_stats.name])
